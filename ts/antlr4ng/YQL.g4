...
select_subexpr: cte_with_clause? select_subexpr_core;
...
pure_column_list: LPAREN an_id (COMMA an_id)* COMMA? RPAREN;
...
cte_with_clause: WITH cte_binding (COMMA cte_binding)* COMMA?;

cte_binding: RECURSIVE? cte_key AS LPAREN cte_value RPAREN;

cte_key: id_table_or_type pure_column_list?;

cte_value: select_stmt | values_stmt;

select_stmt: cte_with_clause? select_stmt_core;

select_stmt_core: select_stmt_intersect (union_op select_stmt_intersect)*;

select_stmt_intersect: select_kind_parenthesis (intersect_op select_kind_parenthesis)*;

select_unparenthesized_stmt: cte_with_clause? select_unparenthesized_stmt_core;

select_unparenthesized_stmt_core: select_unparenthesized_stmt_intersect (union_op select_stmt_intersect)*;
...
select_kind: (DISCARD)? (process_core | reduce_core | select_core | combine_core) (INTO RESULT pure_column_or_named)?;
...
materialize_stmt: MATERIALIZE named_single_source INTO bind_parameter (ON cluster_expr)? table_hints?;
...
grouping_sets_specification: GROUPING SETS LPAREN grouping_element_list COMMA? RPAREN;
...
named_single_source: hinted_single_source row_pattern_recognition_clause? ((AS an_id | an_id_as_compat) pure_column_list?)? (sample_clause | tablesample_clause)?;
...
values_source_row_list: values_source_row (COMMA values_source_row)* COMMA?;
...
create_view_stmt: CREATE VIEW (IF NOT EXISTS)? simple_table_ref_core
...
object_features: object_feature | LPAREN object_feature (COMMA object_feature)* COMMA? RPAREN;
...
backup_collection_settings: backup_collection_settings_entry (COMMA backup_collection_settings_entry)* COMMA?;
...
with_truncate_table_settings: WITH LPAREN RPAREN;
...
with_table_settings: WITH LPAREN table_settings_entry (COMMA table_settings_entry)* COMMA? RPAREN;
...
alter_table_action:
  ...
  | alter_table_add_statistics
  | alter_table_drop_statistics
  | alter_table_rebuild_index
;
...
alter_table_alter_column_set_not_null: ALTER COLUMN an_id SET NOT NULL;
alter_table_alter_column_set_compression: ALTER COLUMN an_id SET compression;
...
alter_table_add_statistics: ADD table_statistics;
alter_table_drop_statistics: DROP STATISTICS an_id;
...
alter_table_rebuild_index: REBUILD INDEX an_id
    with_index_settings?
;
alter_table_compact: COMPACT with_compact_settings?;
alter_table_alter_column_set_encoding: ALTER COLUMN an_id SET encoding;

column_schema: an_id_schema type_name_or_bind column_option_list;
alter_table_alter_column_set_default: ALTER COLUMN an_id SET default_value;
alter_table_alter_column_drop_default: ALTER COLUMN an_id DROP DEFAULT;

column_option_list: column_option_list_space | column_option_list_comma;
column_option_list_space: (column_option)*;
column_option_list_comma: LPAREN column_option (COMMA column_option)* RPAREN;
...
compression_setting_entry: an_id EQUALS compression_setting_value;
compression_setting_value: integer | id;
...
encoding_config: encoding_config_name (LPAREN (encoding_setting_entry (COMMA encoding_setting_entry)*)? COMMA? RPAREN)?;
encoding_config_name: an_id_or_type;
encoding_setting_entry: an_id EQUALS encoding_setting_value;
encoding_setting_value: integer | id;
generated_always: (GENERATED ALWAYS)? AS LPAREN expr RPAREN (STORED | VIRTUAL)?;
...
table_statistics: STATISTICS an_id
    ON LPAREN an_id_schema (COMMA an_id_schema)* RPAREN
    with_statistics_types?
;

with_statistics_types: WITH LPAREN an_id_or_type (COMMA an_id_or_type)* COMMA? RPAREN;

with_compact_settings: WITH LPAREN compact_setting_entry (COMMA compact_setting_entry)* COMMA? RPAREN;
compact_setting_entry: an_id EQUALS compact_setting_value;
compact_setting_value: literal_value;

changefeed: CHANGEFEED an_id WITH LPAREN changefeed_settings RPAREN;
changefeed_settings: changefeed_settings_entry (COMMA changefeed_settings_entry)* COMMA?;
...
replication_settings: replication_settings_entry (COMMA replication_settings_entry)* COMMA?;
...
transfer_settings: transfer_settings_entry (COMMA transfer_settings_entry)* COMMA?;
...
table_ref: (cluster_expr DOT)? COMMAT? (table_key | an_id_expr LPAREN (table_arg (COMMA table_arg)* COMMA?)? RPAREN | bind_parameter (LPAREN expr_list? RPAREN)? (VIEW view_name)?);

table_key: id_table_or_type (VIEW view_name)?;
table_arg: COMMAT? named_expr (VIEW view_name)?;
table_hints: WITH (table_hint | LPAREN table_hint (COMMA table_hint)* COMMA? RPAREN);
table_hint:
...
topic_settings: topic_settings_entry (COMMA topic_settings_entry)* COMMA?;
...
topic_consumer_settings: topic_consumer_settings_entry (COMMA topic_consumer_settings_entry)* COMMA?;
